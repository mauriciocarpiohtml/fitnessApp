...
function EjercicioCard({ejercicio}) {
  ...
  return (
    <div className='p-3 flex flex-col border-t-2 border-teal-700  shadow-lg cursor-pointer transition duration-500 ease-in-out transform hover:scale-110'
          onClick={()=> activarModal(ejercicio)}>
      <img src={gifUrl} alt={name} />

      <p className='uppercase text-stone-600 text-center mt-1'>{name}</p>
        <div className='flex flex-col md:flex md:flex-row gap-3 md:gap-5 mx-auto mt-3 mb-5'>
            <button className='p-2 bg-teal-600 text-white rounded-full mb-3 md:mb-0 md:mr-3'>
                {bodyPart}
            </button>
